const elements = {
  ...
  codexCloudEnvironmentsText: document.querySelector("#codex-cloud-environments-text"),
  codexCloudEnvironmentsValuesText: document.querySelector(
    "#codex-cloud-environments-values-text",
  ),
  ...
  accountRateLimitsButton: document.querySelector("#account-rate-limits-button"),
  accountRateLimitsStatus: document.querySelector("#account-rate-limits-status"),
...
elements.workspaceSelect.addEventListener("change", () => {
  ...
  elements.accountRateLimitsStatus.textContent = "Limit read disabled by default.";
...
elements.accountRateLimitsButton.addEventListener("click", () => {
...
function accountRateLimitsEndpoint() {
  if (!selectedWorkspaceId) {
    return "/api/account-rate-limits";
  }
...
async function runAccountRateLimits() {
  setAccountRateLimitsLoading(true);
  hideError();

  try {
    const response = await fetch(accountRateLimitsEndpoint(), {
      headers: apiHeaders(),
      cache: "no-store",
    });
    const payload = await response.json();
    ...
    renderAccountRateLimits(payload);
  } catch (error) {
    elements.accountRateLimitsStatus.textContent = "Failed";
    renderError(error);
  } finally {
...
function renderSettingsIntegrations(payload) {
  ...
  elements.codexAgentInternetAccessValuesText.textContent =
    ...
    codexAgentInternetAccess.appServerTraffic
      ? "Returned"
      : "Hidden";
  elements.codexCloudEnvironmentsText.textContent = codexCloudEnvironments.returned
    ? `${codexCloudEnvironments.catalogOnlyEntryCount ?? 0} catalog / ${
        codexCloudEnvironments.entryCount ?? 0
      } tracked`
    : "Blocked";
  elements.codexCloudEnvironmentsValuesText.textContent =
    codexCloudEnvironments.environmentNamesReturned ||
    codexCloudEnvironments.settingsUrlsReturned ||
    codexCloudEnvironments.repositoryNamesReturned ||
    codexCloudEnvironments.branchOrShaValuesReturned ||
    codexCloudEnvironments.setupScriptContentsReturned ||
    codexCloudEnvironments.maintenanceScriptContentsReturned ||
    codexCloudEnvironments.packageManagerValuesReturned ||
    codexCloudEnvironments.runtimeVersionValuesReturned ||
    codexCloudEnvironments.environmentVariableValuesReturned ||
    codexCloudEnvironments.secretValuesReturned ||
    codexCloudEnvironments.imageReferencesReturned ||
    codexCloudEnvironments.commandTextsReturned ||
    codexCloudEnvironments.diffContentsReturned ||
    codexCloudEnvironments.pullRequestUrlsReturned ||
    codexCloudEnvironments.cacheStatesReturned ||
    codexCloudEnvironments.proxyValuesReturned ||
    codexCloudEnvironments.taskOutputsReturned ||
    codexCloudEnvironments.cloudTasksStarted ||
    codexCloudEnvironments.containersCreated ||
    codexCloudEnvironments.repositoriesCheckedOut ||
    codexCloudEnvironments.setupScriptsExecuted ||
    codexCloudEnvironments.maintenanceScriptsExecuted ||
    codexCloudEnvironments.dependenciesInstalled ||
    codexCloudEnvironments.packageVersionsPinned ||
    codexCloudEnvironments.environmentVariablesApplied ||
    codexCloudEnvironments.secretsDecrypted ||
    codexCloudEnvironments.cacheResets ||
    codexCloudEnvironments.pullRequestsOpened ||
    codexCloudEnvironments.followupsStarted ||
    codexCloudEnvironments.networkProxiesConfigured ||
    codexCloudEnvironments.agentCommandsRun ||
    codexCloudEnvironments.filesystemReads ||
    codexCloudEnvironments.filesystemWrites ||
    codexCloudEnvironments.networkAccess ||
    codexCloudEnvironments.modelTraffic ||
    codexCloudEnvironments.mutationEnabled ||
    codexCloudEnvironments.pathsReturned ||
    codexCloudEnvironments.urlsReturned ||
    codexCloudEnvironments.secretsReturned ||
    codexCloudEnvironments.rawPayloadsReturned ||
    codexCloudEnvironments.appServerTraffic
      ? "Returned"
      : "Hidden";
  ...
  elements.skillsPluginsCatalogValuesText.textContent =
    ...
    skillsPluginsCatalog.appServerTraffic
      ? "Returned"
      : "Hidden";
  elements.codexPluginBuildText.textContent = codexPluginBuild.returned
    ? `${codexPluginBuild.catalogOnlyEntryCount ?? 0} catalog / ${
        codexPluginBuild.entryCount ?? 0
      } tracked`
    : "Blocked";
  elements.codexPluginBuildValuesText.textContent =
    codexPluginBuild.pluginManifestsReturned ||
    codexPluginBuild.pluginNamesReturned ||
    codexPluginBuild.pluginVersionsReturned ||
    codexPluginBuild.pluginDescriptionsReturned ||
    codexPluginBuild.skillNamesReturned ||
    codexPluginBuild.skillContentReturned ||
    codexPluginBuild.mcpConfigsReturned ||
    codexPluginBuild.appIntegrationsReturned ||
    codexPluginBuild.marketplaceFilesReturned ||
    codexPluginBuild.marketplaceEntriesReturned ||
    codexPluginBuild.marketplaceNamesReturned ||
    codexPluginBuild.marketplaceSourcesReturned ||
    codexPluginBuild.marketplaceLocatorsReturned ||
    codexPluginBuild.displayNamesReturned ||
    codexPluginBuild.localPluginPathsReturned ||
    codexPluginBuild.workspacePrincipalsReturned ||
    codexPluginBuild.shareLinksReturned ||
    codexPluginBuild.adminRequirementsReturned ||
    codexPluginBuild.commandTextReturned ||
    codexPluginBuild.scaffoldsExecuted ||
    codexPluginBuild.marketplacesWritten ||
    codexPluginBuild.manifestsWritten ||
    codexPluginBuild.pluginsCopied ||
    codexPluginBuild.pluginsShared ||
    codexPluginBuild.marketplaceCliExecuted ||
    codexPluginBuild.externalCodeMaterialized ||
    codexPluginBuild.filesystemReads ||
    codexPluginBuild.filesystemWrites ||
    codexPluginBuild.mutationEnabled ||
    codexPluginBuild.pathsReturned ||
    codexPluginBuild.urlsReturned ||
  ...
  elements.appProfileText.textContent = profile.returned
    ? `${profile.catalogOnlySettingCount ?? 0} catalog / ${
        profile.settingCount ?? 0
      } tracked`
    : "Blocked";
  elements.appProfileValuesText.textContent =
    profile.activityMetricsReturned ||
    profile.tokenValuesReturned ||
    profile.profileDetailsReturned ||
    profile.profilePicturesReturned ||
    profile.displayNamesReturned ||
    profile.usernamesReturned ||
    profile.profileCardsReturned ||
    profile.invitationEligibilityReturned ||
  ...
  elements.accountRateLimitsStatus.textContent = auth.accountRateLimitsEnabled
    ? "Limit read enabled"
    : "Limit read disabled";
...
function renderAccountRateLimits(payload) {
  const auth = payload.auth ?? {};
  const result = payload.result ?? {};
  const blocked = auth.accountRateLimitsEnabled !== true || payload.appServer?.touched !== true;
  const bucketCount = result.bucketCount ?? payload.probes?.accountRateLimits?.bucketCount ?? 0;
  const status = result.status ?? (blocked ? "blocked" : "available");
  elements.accountRateLimitsStatus.textContent = blocked ? "Read blocked" : status;
  elements.accountRateLimitsBucketsText.textContent = String(bucketCount);
  elements.accountRateLimitsStateText.textContent = result.rateLimitReached ? "Limited" : status;
  elements.accountRateLimitsDetailsText.textContent =
    result.rateLimitDetailsReturned || auth.rawPayloadReturned ? "Returned" : "Hidden";
...
function renderCodexCloudEnvironmentsCatalog(summary) {
  elements.codexCloudEnvironmentsList.replaceChildren();
  const entries = Array.isArray(summary?.entries) ? summary.entries : [];
  if (entries.length === 0) {
    elements.codexCloudEnvironmentsList.append(
      emptyState("No Codex cloud environments catalog returned."),
    );
    return;
  }
  ...
  for (const entry of entries) {
    ...
    const meta = document.createElement("span");
    meta.textContent = entry.group ?? "cloud";
    ...
    for (const value of [
      ...
      entry.environmentNameReturned
        ? "environment names returned"
        : "environment names hidden",
      entry.settingsUrlReturned ? "settings URLs returned" : "settings URLs hidden",
      entry.repositoryNameReturned ? "repository names returned" : "repository names hidden",
      entry.branchOrShaReturned ? "branch/SHA values returned" : "branch/SHA values hidden",
      entry.setupScriptContentReturned
        ? "setup script content returned"
        : "setup script content hidden",
      entry.maintenanceScriptContentReturned
        ? "maintenance script content returned"
        : "maintenance script content hidden",
      entry.packageManagerValueReturned
        ? "package manager values returned"
        : "package manager values hidden",
      entry.runtimeVersionValueReturned
        ? "runtime version values returned"
        : "runtime version values hidden",
      entry.environmentVariableValueReturned
        ? "environment variable values returned"
        : "environment variable values hidden",
      entry.secretValueReturned ? "secret values returned" : "secret values hidden",
      entry.imageReferenceReturned ? "image references returned" : "image references hidden",
      entry.commandTextReturned ? "command text returned" : "command text hidden",
      entry.diffContentReturned ? "diff content returned" : "diff content hidden",
      entry.pullRequestUrlReturned ? "PR URLs returned" : "PR URLs hidden",
      entry.cacheStateReturned ? "cache state returned" : "cache state hidden",
      entry.proxyValueReturned ? "proxy values returned" : "proxy values hidden",
      entry.taskOutputReturned ? "task output returned" : "task output hidden",
      entry.cloudTaskStarted ? "cloud task started" : "cloud tasks blocked",
      entry.containerCreated ? "container created" : "container creation blocked",
      entry.repositoryCheckedOut ? "repository checked out" : "repository checkout blocked",
      entry.setupScriptExecuted ? "setup script executed" : "setup script execution blocked",
      entry.maintenanceScriptExecuted
        ? "maintenance script executed"
        : "maintenance script execution blocked",
      entry.dependenciesInstalled ? "dependencies installed" : "dependency installs blocked",
      entry.packageVersionsPinned ? "package versions pinned" : "package pinning blocked",
      entry.environmentVariablesApplied
        ? "environment variables applied"
        : "environment variable application blocked",
      entry.secretsDecrypted ? "secrets decrypted" : "secret decryption blocked",
      entry.cacheReset ? "cache reset" : "cache reset blocked",
      entry.pullRequestOpened ? "PR opened" : "PR creation blocked",
      entry.followupStarted ? "follow-up started" : "follow-ups blocked",
      entry.networkProxyConfigured ? "network proxy configured" : "network proxy blocked",
      entry.agentCommandsRun ? "agent commands run" : "agent commands blocked",
      entry.filesystemRead ? "filesystem read" : "filesystem reads blocked",
      entry.filesystemWrite ? "filesystem written" : "filesystem writes blocked",
      entry.networkAccess ? "network access" : "network blocked",
      entry.modelTraffic ? "model traffic" : "model traffic blocked",
    ...
    header.append(title, meta);
    row.append(header, chips);
    elements.codexCloudEnvironmentsList.append(row);
  }
}
...
function renderCodexPluginBuildCatalog(summary) {
  elements.codexPluginBuildList.replaceChildren();
  const entries = Array.isArray(summary?.entries) ? summary.entries : [];
  if (entries.length === 0) {
    elements.codexPluginBuildList.append(
      emptyState("No Codex Plugin Build catalog returned."),
    );
    return;
  }
  ...
  for (const entry of entries) {
    ...
    const meta = document.createElement("span");
    meta.textContent = entry.group ?? "plugin-build";
    ...
    for (const value of [
      entry.state ?? "blocked",
      entry.source ?? null,
      entry.pluginManifestReturned ? "manifests returned" : "manifests hidden",
      entry.pluginNameReturned ? "plugin names returned" : "plugin names hidden",
      entry.pluginVersionReturned ? "versions returned" : "versions hidden",
      entry.pluginDescriptionReturned ? "descriptions returned" : "descriptions hidden",
      entry.skillNameReturned ? "skill names returned" : "skill names hidden",
      entry.skillContentReturned ? "skill content returned" : "skill content hidden",
      entry.mcpConfigReturned ? "MCP configs returned" : "MCP configs hidden",
      entry.appIntegrationReturned ? "app integrations returned" : "app integrations hidden",
      entry.marketplaceFileReturned ? "marketplace files returned" : "marketplace files hidden",
      entry.marketplaceEntryReturned
        ? "marketplace entries returned"
        : "marketplace entries hidden",
      entry.marketplaceNameReturned ? "marketplace names returned" : "marketplace names hidden",
      entry.marketplaceSourceReturned
        ? "marketplace sources returned"
        : "marketplace sources hidden",
      entry.marketplaceLocatorReturned
        ? "marketplace locators returned"
        : "marketplace locators hidden",
      entry.displayNameReturned ? "display names returned" : "display names hidden",
      entry.localPluginPathReturned ? "local plugin paths returned" : "local plugin paths hidden",
      entry.workspacePrincipalReturned
        ? "workspace principals returned"
        : "workspace principals hidden",
      entry.shareLinkReturned ? "share links returned" : "share links hidden",
      entry.adminRequirementReturned
        ? "admin requirements returned"
        : "admin requirements hidden",
      entry.commandTextReturned ? "command text returned" : "command text hidden",
      entry.scaffoldExecuted ? "scaffold executed" : "scaffold blocked",
      entry.marketplaceWritten ? "marketplace written" : "marketplace write blocked",
      entry.manifestWritten ? "manifest written" : "manifest write blocked",
      entry.pluginCopied ? "plugin copied" : "plugin copy blocked",
      entry.pluginShared ? "plugin shared" : "plugin share blocked",
      entry.marketplaceCliExecuted ? "marketplace CLI executed" : "marketplace CLI blocked",
      entry.externalCodeMaterialized
        ? "external code materialized"
        : "external code blocked",
      entry.filesystemRead ? "filesystem read" : "filesystem reads blocked",
      entry.filesystemWrite ? "filesystem write" : "filesystem writes blocked",
      entry.mutationEnabled ? "mutation enabled" : "mutation blocked",
      entry.pathsReturned ? "paths returned" : "paths hidden",
...
function renderCodexAppProfileSettings(summary) {
  elements.appProfileList.replaceChildren();
  const settings = Array.isArray(summary?.settings) ? summary.settings : [];
  if (settings.length === 0) {
    elements.appProfileList.append(emptyState("No profile settings catalog returned."));
    return;
  }
  for (const setting of settings) {
    const row = document.createElement("article");
    ...
    const meta = document.createElement("span");
    meta.textContent = setting.group ?? "profile";
    ...
    for (const value of [
      ...
      setting.source ?? null,
      setting.settingValueReturned ? "value returned" : "value hidden",
      setting.activityMetricReturned ? "metric returned" : "metric hidden",
      setting.tokenValueReturned ? "token value returned" : "token value hidden",
      setting.profileDetailReturned ? "profile detail returned" : "profile detail hidden",
      setting.profilePictureReturned ? "picture returned" : "picture hidden",
      setting.displayNameReturned ? "display name returned" : "display name hidden",
      setting.usernameReturned ? "username returned" : "username hidden",
      setting.profileCardReturned ? "profile card returned" : "profile card hidden",
      setting.invitationEligibilityReturned ? "eligibility returned" : "eligibility hidden",
      setting.invitationSent ? "invitation sent" : "invitation blocked",
      setting.appServerTraffic ? "app-server traffic" : "local catalog",
    ]) {
      if (!value) {
        continue;
...
function setAccountRateLimitsLoading(isLoading) {
  elements.accountRateLimitsButton.disabled = isLoading;

...
export async function runIntegrationsInventoryProbe({
  ...
  try {
    ...
    const [
      ...
      readInventorySection(() =>
        client.request(APP_SERVER_METHODS.skillsList, {
          cwds: [cwd],
          forceReload: false,
        }),
      ),
      readInventorySection(() =>
        client.request(APP_SERVER_METHODS.pluginList, {
          cwds: [cwd],
...
export async function runAccountUsageReadProbe({
  codexBin = process.env.CODEX_BIN || "codex",
  codexArgs = ["app-server", "--listen", "stdio://"],
  cwd = process.cwd(),
  timeoutMs = DEFAULT_TIMEOUT_MS,
  ...
  if (process.env.CODEX_APP_PORT_ALLOW_ACCOUNT_USAGE !== "1") {
    throw new Error(
      "account/usage/read requires CODEX_APP_PORT_ALLOW_ACCOUNT_USAGE=1 because it inspects local account activity state",
    );
  }
  ...
  try {
    ...
    client.notify(APP_SERVER_METHODS.initialized);
    const accountUsage = await client.request(
      APP_SERVER_METHODS.accountUsageRead,
      null,
      { timeoutMs },
    );
    ...
    return {
      ...
      initialize: summarizeInitialize(initialize),
      probes: {
        accountUsage: summarizeAccountUsageInventory({ ok: true, result: accountUsage }),
      },
      notifications: notificationCounts(notifications),
    };
  } finally {
...
export async function runCollaborationModesReadProbe({
  codexBin = process.env.CODEX_BIN || "codex",
  codexArgs = ["app-server", "--listen", "stdio://"],
  cwd = process.cwd(),
  timeoutMs = DEFAULT_TIMEOUT_MS,
  ...
  if (process.env.CODEX_APP_PORT_ALLOW_COLLABORATION_MODES !== "1") {
    throw new Error(
      "collaborationMode/list requires CODEX_APP_PORT_ALLOW_COLLABORATION_MODES=1 because it inspects collaboration mode presets",
    );
  }
  const notifications = [];
  const client = new JsonlRpcClient({
  ...
  try {
    ...
    client.notify(APP_SERVER_METHODS.initialized);
    const collaborationModes = await client.request(
    ...
    return {
      ...
      initialize: summarizeInitialize(initialize),
      probes: {
        collaborationModes: summarizeCollaborationModesInventory(
          {
            ok: true,
...
export async function runRemoteControlStatusReadProbe({
  codexBin = process.env.CODEX_BIN || "codex",
  codexArgs = ["app-server", "--listen", "stdio://"],
  cwd = process.cwd(),
  timeoutMs = DEFAULT_TIMEOUT_MS,
  ...
  if (process.env.CODEX_APP_PORT_ALLOW_REMOTE_CONTROL_STATUS !== "1") {
    throw new Error(
      "remoteControl/status/read requires CODEX_APP_PORT_ALLOW_REMOTE_CONTROL_STATUS=1 because it inspects local remote-control connection state",
    );
  }
  ...
  try {
    ...
    client.notify(APP_SERVER_METHODS.initialized);
    const remoteControlStatus = await client.request(
    ...
    return {
      ...
      initialize: summarizeInitialize(initialize),
      probes: {
        remoteControlStatus: summarizeRemoteControlStatusInventory({
          ok: true,
          result: remoteControlStatus,
        }),
      },
      notifications: notificationCounts(notifications),
    };
...
export async function runInstalledPluginsReadProbe({
  codexBin = process.env.CODEX_BIN || "codex",
  codexArgs = ["app-server", "--listen", "stdio://"],
  cwd = process.cwd(),
  timeoutMs = DEFAULT_TIMEOUT_MS,
  ...
  if (process.env.CODEX_APP_PORT_ALLOW_INSTALLED_PLUGINS !== "1") {
    throw new Error(
      "plugin/installed requires CODEX_APP_PORT_ALLOW_INSTALLED_PLUGINS=1 because it inspects local installed plugin state",
    );
  }
  ...
  try {
    ...
    client.notify(APP_SERVER_METHODS.initialized);
    const installedPlugins = await client.request(APP_SERVER_METHODS.pluginInstalled, {
      cwds: [cwd],
      installSuggestionPluginNames: null,
    });
    ...
    return {
      ...
      initialize: summarizeInitialize(initialize),
      probes: {
        installedPlugins: summarizeInstalledPluginsInventory(
          {
            ok: true,
...
export async function runPluginsListReadProbe({
  codexBin = process.env.CODEX_BIN || "codex",
  codexArgs = ["app-server", "--listen", "stdio://"],
  cwd = process.cwd(),
  timeoutMs = DEFAULT_TIMEOUT_MS,
  ...
  if (process.env.CODEX_APP_PORT_ALLOW_PLUGINS_LIST !== "1") {
    throw new Error(
      "plugin/list requires CODEX_APP_PORT_ALLOW_PLUGINS_LIST=1 because it inspects local plugin marketplace inventory metadata",
    );
  }
  const notifications = [];
  const client = new JsonlRpcClient({
  ...
  try {
    ...
    return {
      ...
      initialize: summarizeInitialize(initialize),
      probes: {
        plugins: summarizePluginsInventory(
          {
            ok: true,
            result: plugins,
          },
...
export async function runExternalAgentConfigImportHistoriesReadProbe({
  codexBin = process.env.CODEX_BIN || "codex",
  codexArgs = ["app-server", "--listen", "stdio://"],
  cwd = process.cwd(),
  timeoutMs = DEFAULT_TIMEOUT_MS,
  ...
  if (process.env.CODEX_APP_PORT_ALLOW_EXTERNAL_AGENT_IMPORT_HISTORIES !== "1") {
    throw new Error(
      "externalAgentConfig/import/readHistories requires CODEX_APP_PORT_ALLOW_EXTERNAL_AGENT_IMPORT_HISTORIES=1 because it inspects local import history metadata",
    );
  }
  ...
  try {
    ...
    client.notify(APP_SERVER_METHODS.initialized);
    const importHistories = await client.request(
      APP_SERVER_METHODS.externalAgentConfigImportReadHistories,
      null,
      { timeoutMs },
    );
...
export async function runConfigRequirementsReadProbe({
  codexBin = process.env.CODEX_BIN || "codex",
  codexArgs = ["app-server", "--listen", "stdio://"],
  cwd = process.cwd(),
  timeoutMs = DEFAULT_TIMEOUT_MS,
  ...
  if (process.env.CODEX_APP_PORT_ALLOW_CONFIG_REQUIREMENTS !== "1") {
    throw new Error(
      "configRequirements/read requires CODEX_APP_PORT_ALLOW_CONFIG_REQUIREMENTS=1 because it inspects managed configuration requirement metadata",
    );
  }
  ...
  try {
    ...
    const requirements = await client.request(
      APP_SERVER_METHODS.configRequirementsRead,
      null,
      { timeoutMs },
    );
    ...
    return {
      ...
      initialize: summarizeInitialize(initialize),
      probes: {
        configRequirements: summarizeConfigRequirementsInventory({
          ok: true,
          result: requirements,
        }),
      },
...
export async function runMcpServerStatusReadProbe({
  codexBin = process.env.CODEX_BIN || "codex",
  codexArgs = ["app-server", "--listen", "stdio://"],
  cwd = process.cwd(),
  timeoutMs = DEFAULT_TIMEOUT_MS,
  ...
  if (process.env.CODEX_APP_PORT_ALLOW_MCP_SERVER_STATUS !== "1") {
    throw new Error(
      "mcpServerStatus/list requires CODEX_APP_PORT_ALLOW_MCP_SERVER_STATUS=1 because it inspects MCP server metadata",
    );
  }
  ...
  try {
    ...
    client.notify(APP_SERVER_METHODS.initialized);
    const mcpServerStatus = await client.request(
      APP_SERVER_METHODS.mcpServerStatusList,
      {
        detail: "toolsAndAuthOnly",
        limit: 50,
      },
    ...
    return {
      ...
      initialize: summarizeInitialize(initialize),
      probes: {
        mcpServerStatus: summarizeMcpInventory(
          {
            ok: true,
...
export async function runSkillsListReadProbe({
  codexBin = process.env.CODEX_BIN || "codex",
  codexArgs = ["app-server", "--listen", "stdio://"],
  cwd = process.cwd(),
  timeoutMs = DEFAULT_TIMEOUT_MS,
  ...
  if (process.env.CODEX_APP_PORT_ALLOW_SKILLS_LIST !== "1") {
    throw new Error(
      "skills/list requires CODEX_APP_PORT_ALLOW_SKILLS_LIST=1 because it inspects local skill inventory metadata",
    );
  }
  const notifications = [];
  const client = new JsonlRpcClient({
  ...
  try {
    ...
    return {
      ...
      initialize: summarizeInitialize(initialize),
      probes: {
        skills: summarizeSkillsInventory(
          {
            ok: true,
            result: skills,
          },
...
export async function runHooksListReadProbe({
  codexBin = process.env.CODEX_BIN || "codex",
  codexArgs = ["app-server", "--listen", "stdio://"],
  cwd = process.cwd(),
  timeoutMs = DEFAULT_TIMEOUT_MS,
  ...
  if (process.env.CODEX_APP_PORT_ALLOW_HOOKS_LIST !== "1") {
    throw new Error(
      "hooks/list requires CODEX_APP_PORT_ALLOW_HOOKS_LIST=1 because it inspects local hook configuration metadata",
    );
  }
  ...
  try {
    ...
    client.notify(APP_SERVER_METHODS.initialized);
    const hooks = await client.request(APP_SERVER_METHODS.hooksList, {
      cwds: [cwd],
    });
...
function summarizeAccountUsageInventory(section) {
  ...
  return {
    method: APP_SERVER_METHODS.accountUsageRead,
    ok: section.ok,

...
- Browser-facing approval-decision routes may record local deny-only decisions
  ...
  plus local boundary keys, groups, states, sources, counts, and redaction
  flags, including marketplace source classes, sparse-checkout guidance, ref
  pinning, configured marketplace snapshots, bundled app integrations/lifecycle
  hooks, local-skill-first authoring guidance, stable package publishing,
  workspace group sharing, and workspace/org boundaries; it
...
- `/api/installed-plugins` may call only `plugin/installed` behind
  ...
  blocked.
- `/api/external-agent-import-histories` may call only
  `externalAgentConfig/import/readHistories` behind
  `CODEX_APP_PORT_ALLOW_EXTERNAL_AGENT_IMPORT_HISTORIES=1`. It is GET-only,
  local-token protected, and returns history/success/failure counts,
  field-presence counts, and allowlisted item-type buckets only; import ids,
